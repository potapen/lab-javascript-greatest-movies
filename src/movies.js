...
function getAllDirectors(moviesArray) {
  ...
  console.log('the directors are: ', directorsArray)
  return directorsArray
}
// getAllDirectors(movies)

// Iteration 2: Steven Spielberg. The best? - How many drama movies did STEVEN SPIELBERG direct?
function howManyMovies(moviesArray,director='Steven Spielberg') {
  const moviesFromDirector = moviesArray.filter(movie => movie.director===director && movie.genre.includes('Drama'))
  console.log(moviesFromDirector)
  const moviesCount = moviesFromDirector.length
  console.log(`there are ${moviesCount} drama movies from director ${director}.`)
  return moviesCount
}
//howManyMovies(movies, 'Steven Spielberg')

//To test the test function
// howManyMovies([
//   {
//     director: 'Steven Spielberg',
//     genre: ['Action', 'Drama', 'Thriller']
...
function scoresAverage(moviesArray) {
  ...
  const sumRating = moviesArray.reduce( (acc,movie) => acc += movie.score?movie.score:0, 0)
  const count = moviesArray.length
  const avg = Number((sumRating/count).toFixed(2))
  console.log(`sumRating: ${sumRating}, count: ${count}, avg: ${avg}`)
  return avg
}
...
function dramaMoviesScore(moviesArray) {
  ...
  dramaAvg = scoresAverage(dramaArray)
  console.log('average score from dramas is: ', dramaAvg)
  return dramaAvg
}
// dramaMoviesScore(movies)
// Iteration 5: Ordering by year - Order by year, ascending (in growing order)
function orderByYear(moviesArray) {
  const moviesSortedByYear = [...moviesArray]
  moviesSortedByYear.sort(
    (movie1, movie2) => movie1.year!==movie2.year ? movie1.year-movie2.year : movie1.title.localeCompare(movie2.title)
    //if the year is different sort according to the year
    //else sort according to the title using localeCompare
  )
  console.log('the moviesSortedByYear :', moviesSortedByYear)
...
function orderAlphabetically(moviesArray) {
  const moviesSortedAlpha = [...moviesArray]
  moviesSortedAlpha.sort(
    (movie1, movie2) =>  movie1.title.localeCompare(movie2.title)
  )
  const moviesName = moviesSortedAlpha.map( movies => movies.title)
  return moviesName.length>20 ? moviesName.slice(0,20) : moviesName
}
//orderAlphabetically(movies)

// BONUS - Iteration 7: Time Format - Turn duration of the movies from hours to minutes
function turnHoursToMinutes(moviesArray) {
  // below is not working because of shallow copy
  // const moviesMinutes = [...moviesArray]
  // moviesMinutes.forEach( (movies,index) => {
  //   const time = movies.duration
  //   const resultat = time.match(/(\d)+h (\d+)min/)
  //   const hour = resultat[1]
  //   const minute = resultat[2]
  //   moviesMinutes[index].duration = Number(hour)*60 + Number(minute)
  //   }
  // )
  const moviesMinutes = moviesArray.map( (movie, index) => {
    const movieMinute = Object.assign({}, movie)
    const time = movieMinute.duration
    //the time is written as 3h 34min OR 2h
    const resultat = time.match(/(\d+)h\s?(\d*)/)

    const hour = resultat[1]
    const minuteTemp = resultat[2]
    const minute = minuteTemp ? minuteTemp : 0
    movieMinute.duration = Number(hour)*60 + Number(minute)
...
console.log('----------------------------')
console.log(turnHoursToMinutes(movies) instanceof Array)
